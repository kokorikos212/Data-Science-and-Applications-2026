...
class UniverseVisualizer:
    ...
    def plot_missing_values(self, figsize=(12, 6), threshold=20):
        """
        Visualizes the percentage of missing values per feature.
        
        Args:
            figsize (tuple): Dimensions of the plot.
            threshold (int): Percentage value to draw a 'danger' line.
        """
        # Calculate percentage of NaNs per column
        missing_pct = (self.df.isnull().sum() / len(self.df)) * 100
        
        # Sort and filter out columns with 0 missing values for clarity
        missing_pct = missing_pct[missing_pct > 0].sort_values(ascending=False)

        if missing_pct.empty:
            print("Perfect Data: No missing values detected in the manifest.")
            return

        plt.figure(figsize=figsize)
        
        # Use a barplot to show the sparsity of each feature
        ax = sns.barplot(x=missing_pct.index, y=missing_pct.values)
        
        # Add a threshold line to guide the feature selection process
        plt.axhline(y=threshold, color='crimson', linestyle='--', label=f'{threshold}% Threshold')
        
        plt.xticks(rotation=45, ha='right')
        plt.title('Missing Values Percentage per Gaia Feature')
        plt.ylabel('Missing Percentage (%)')
        plt.xlabel('Features')
        plt.legend()
        plt.show()

        print(f"Data Profiling: {len(missing_pct)} features contain missing values.")
